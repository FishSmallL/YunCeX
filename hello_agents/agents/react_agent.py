...
class ReActAgent(Agent):
    ...
    def _run_impl(self, input_text: str, session_start_time, **kwargs) -> str:
        ...
        while current_step < self.max_steps:
            ...
            for tool_call in tool_calls:
                ...
                # 如果工具名为空，直接跳过
                if not tool_name:
                    print("❌ 工具解析失败：tool_name 为空")
                    messages.append({
                        "role": "tool",
                        "tool_call_id": tool_call_id or "unknown",
                        "content": "错误：工具名称为空，无法执行"
                    })
                    continue
                # 记录工具调用
                if self.trace_logger:
    ...
    def _parse_tool_call(self, tool_call):
        """
        统一兼容：
        1. 老版本对象格式（tool_call.id）
        2. 新版本 dict 格式（tool_call["id"]）
        """

        # 新版：dict 格式（DeepSeek / OpenAI 新接口）
    ...
    async def _execute_tools_async(
        ...
        for tc in tool_calls:
            ...
            if parsed["name"] in self._builtin_tools:
            ...
            while current_step < self.max_steps:
                ...
                try:
                    ...
                    for tool_name, tool_call_id, result_dict in tool_results:
                        ...
                        # 检查是否是 Finish 工具
                        if tool_name == "Finish":
                            final_answer = result_dict.get("content", "")
        ...
        if user_calls:
            ...
            async def execute_one(tc):
                async with semaphore:
                    ...
                    tool_name = parsed["name"]
                    tool_call_id = parsed["id"] or "unknown"
                    arguments = parsed["arguments"]
            ...
            user_results = await asyncio.gather(
                *[execute_one(tc) for tc in user_calls]
            )
