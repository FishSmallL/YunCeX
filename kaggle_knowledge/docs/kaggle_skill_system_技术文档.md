...
> 记录 2026-05-11 ~ 2026-05-14 期间对 `kaggle_knowledge` 和 `hello_agents` 体系的重要修改。
...
main.py (ReActAgent 主控)
  ...
  └─ KernelSkillAgent 内部流程:
        1. 解析输入 → 支持逗号分隔多关键词
        2. 对每个关键词循环处理 (_process_one_keyword_stream):
           a. 匹配 Skill 库 (match_keyword)
              ├─ 匹配到且 skill 数量 ≥ min_skills_per_keyword → 紧凑摘要，skip
              └─ 不足或未匹配 → 继续
           b. 检查本地 kernel 数 (对比 min_kernels_per_keyword)
              └─ 不足 → Kaggle 下载
           c. 两阶段 LLM 提取 (每个 kernel 立即保存):
              Phase 1: 亮点扫描 → 无亮点则跳过 Phase 2
              Phase 2: 深度提取
        3. 跨关键词去重 (精确 + 语义)
        4. 按关键词分组重新保存 (覆盖即时保存的预去重版)
        5. 返回紧凑摘要 (每个关键词 top_skills 条，只含名称+影响力+分类+一句话描述)
```
...
`_parse_input()` 按逗号拆分输入，返回 `[(keyword, kernel_dirs), ...]` 列表:
...
`arun_stream()` 和 `run()` 均循环处理每个关键词，通过 `_process_one_keyword_stream()` (异步) 或 `_run_one_keyword()` (同步) 处理。

### 3.2 `_process_one_keyword_stream()` — 异步单关键词 pipeline

每个关键词的独立处理流程:

```
match_keyword → search_skills 数量检查
  ├─ skill_count ≥ min_skills → _build_compact_skill_summary → AGENT_FINISH(skip=True)
  └─ 不足 → 检查本地 kernel → 下载补充 → Phase1/Phase2 提取 → 立即保存
```

`_run_one_keyword()` 是同步版本，返回 `(skills_list, context_or_none)` tuple。
...
Phase 1 无亮点时跳过 Phase 2，不再回退到单 pass 提取。

### 3.4 立即保存机制

每处理完一个 kernel 的 skill 立即保存（`_process_one_keyword_stream` 和 `_run_one_keyword` 内部），防止中途失败全部丢失:
...
提取完成后**不再输出统计摘要**（分类统计、已保存文件列表、库总览等），只调用 `_build_compact_skill_summary()` 返回紧凑 skill 列表:

```
## nlp 方向 — 2 条技巧
1. **skill-name** [高] (模型设计) — 一句话描述
2. **other-skill** [中] (训练策略) — 一句话描述
```

每条只包含: 名称 + 影响力标签 + 分类 + 一句话描述。完整 technique/code/notes 在 skill_library 文件中。

缓存命中路径和提取路径返回**相同格式**，`top_skills` 控制每个关键词返回的条数。

### 3.7 按关键词分组保存

提取+去重后按 `skill["keyword"]` 分组，每个关键词目录只存自己的 skill:
...
for kw, kw_skills in skills_by_kw.items():
    # 清空 kw 目录旧文件 → 保存 kw_skills
```

### 3.8 输出去重

结果**只通过 `AGENT_FINISH.result` 传递**（不再流式输出完整结果），避免在三个位置重复:

```
arun_stream()
  → yield AGENT_FINISH(result=...)     # 唯一传递路径
  → TaskTool 包装为 ToolResponse
  → ReActAgent 观察 / main.py 展示
```

### 3.9 Skill 数量/最小 Kernel 检查

匹配 Skill 库时比较 `skill_count` 和 `min_skills_per_keyword`；下载前检查本地 kernel 数和 `min_kernels_per_keyword`。三个配置项均从 `config.json` 读取。
...
**`SCAN_PROMPT`** — Phase 1 亮点扫描（无亮点时返回 `[]`，跳过 Phase 2）:
```
...
> 注意: `EXTRACTION_PROMPT` 和 `extract_skills_from_notebook()` / `extract_skills_from_notebooks_batch()` 已删除。只保留两阶段提取。

### 5.4 `_extract_json_array()` — 鲁棒 JSON 解析
...
核心机制: 找到所有可能的 `[` 或 `{"name"` 起始位置，用括号平衡计数逐一尝试解析，取最长有效结果。
