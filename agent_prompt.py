...
MAIN_AGENT_PROMPT = f"""
    ...
    ### 六、Checkpoint 与回滚策略
    1. 修改前必须 save_checkpoint, note 必须详细记录：当前模型路径、完整指标、改动内容、理论依据、预期效果。
    2. 若连续两轮训练退化, 必须 list_checkpoints 并 rollback。
    3. rollback 后必须重新验证, 并在 note 中记录回滚原因（哪个改动导致了退化）。
    4. 指标提升超过 0.02 时立即保存, note 中标注"有效改进"。

    ### 七、失败分析与经验学习
    每次训练完成后, 必须做以下分析（通过 read_file 读取文件, 在上下文中理解, 不需要保存到本地）：
    1. 读取 {PROJECT_ROOT}/run_meta.json, 提取 test_metrics 中的 score_a1、f1_a、f1_s、f1_d, 与历史最优对比。
    2. 读取 {PROJECT_ROOT}/output/checkpoints/checkpoint_registry.json, 逐条对比相邻检查点的 note 和 f1_cal：
       - f1 提升的检查点 → 其 note 中描述的改动是"有效策略", 优先复用
       - f1 下降的检查点 → 其 note 中描述的改动是"失败教训", 禁止重复尝试, 除非调整参数后重新评估
       - 连续下降 → 立即回滚到上一个有效检查点
    3. 在 Thought 中总结：本轮学到了什么、哪些策略验证有效、哪些已确认无效。
    4. 在下一轮优化前, 先回顾这些分析结果, 避免重复踩坑。

    ### 八、竞赛技巧增强
    当需要构建或优化模型时, 必须调用：
    其中task要填的是关键词, 选择是和当前任务相关的或者是你觉得对当前任务有启发的关键词, 
    工具会返回与该关键词相关的高分方案分析、代码模式和 Kaggle 竞赛技巧。
    你需要从中提炼出对当前任务最有价值的部分, 结合历史经验进行创新性应用, 而不是盲目套用。
    ``python
