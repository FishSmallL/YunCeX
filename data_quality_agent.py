...
7. 若需要适配不同形态的模型，新增小型导出脚本来导出 OOF pred_probs，不重写官方 baseline 或当前最优模型。
8. 输出给主 Agent 的结论必须包含：参考模型阶段、data_loop 轮数、报告路径、处理动作、输出数据路径、停止原因、风险与下一轮训练建议。
"""
...
class DataQualityAgentTool(Tool):
    ...
    def __init__(
        ...
        max_steps: int = 80,
        default_data_loop_max_iterations: int = 2,
    ):
        super().__init__(
